...
if __name__ == "__main__":
    ...
    envs = ManiSkillVectorEnv(
        envs, args.num_envs, ignore_terminations=not args.partial_reset, **env_kwargs
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            print(
                f"Evaluated {args.num_eval_steps * args.num_envs} steps resulting in {len(eps_lens)} episodes"
            )
            if len(successes) > 0:
            ...
            if writer is not None:
                writer.add_scalar(
                    "charts/eval_episodic_return", returns.mean(), global_step
                )
                writer.add_scalar(
